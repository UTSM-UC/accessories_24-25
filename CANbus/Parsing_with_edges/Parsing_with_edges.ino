...
void sendCANMessage() {
  ...
  unsigned char data_m0[1] = {0x000};  // Data payload for message 1
  ...
  if(state != -1){
      ...
      if (state == 1) {  // If button is pressed
        CAN0.sendMsgBuf(can_id1, 0, 1, data_m1);  // Send message 1 -- opening lights
        Serial.print("Can ID: ");
        ...
        CAN0.sendMsgBuf(can_id1, 0, 1, data_m0);  // Send message 0 -- closing lights
