...
void readCANMessage() {
  ...
  if (CAN0.checkReceive() == CAN_MSGAVAIL || buf[0] & (1 << 1) || buf[0] & (1 << 0)) {
    ...
    bool headlight = (buf[0] & (1 << 0));
    bool leftLightStrip = (buf[0] & (1 << 1));
    bool rightLightStrip = (buf[0] & (1 << 2));
    bool horn = (buf[0] & (1 << 3));
    bool wiper = (buf[0] & (1 << 4));
    bool hazard = (buf[0] & (1 << 5));
...
void sendCANMessage() {
  ...
  sendStates[3] = edgeDetector(3);      // 1 for on 0 for off -1 for unchanged

  if (sendStates[3] != -1) {
    Serial.print("\nSent;     ");

    if (sendStates[3] == 1) {                   // If button is pressed
      canMessage |= (1 << 0);                   // index 0
